...
#  build dataset from the existing pickle
def get_datapoint(path: str, all_graph: dict, args:Namespace):
    targets = []
    crystal_dict = dict()
    with open(path) as f:
        ...
        for line in reader[1:]:
            ...
            targets.append([float(target) for target in line[1:]])

    # construct dataset
    data = CrystalDataset([
        all_graph[name] for name, targets in tqdm(crystal_dict.items(), total=len(crystal_dict))
    ], args=args)

    # print(targets)
    # print(f"original {type(data.targets())}")
    # print(f"original {len(data.targets())}")
    data.set_targets(targets)
    # print(f"after {type(data.targets())}")
    # print(f"after {len(data.targets())}")
    # exit()
    return data
