...
class GaussianDistance(object):
    ...
    def expand(self, distances):
        ...
        # print(f"origin {np.shape(distances)}")
        # print(f"after {np.shape(distances[..., np.newaxis])}")
        # print(f"final {np.shape(np.exp(-(distances[..., np.newaxis] - self.filter) ** 2 / self.var ** 2))}")
        # print(f"filter {np.shape(self.filter)}")

        # print(f"miners {np.shape(-(distances[..., np.newaxis] - self.filter))}")
        # exit()
